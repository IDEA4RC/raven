...
export class CohortSelectionComponent implements OnInit, OnDestroy {
  ...
  goToNLPCohort() {
    ...
    let token = localStorage.getItem('access_token');
    let userId = localStorage.getItem('user_id');
    const url = `https://idea-nlp-cohort-builder.d.lst.tfo.upm.esnpm/?user_id=${userId}&analysis_id=${this.analysisId}&permit_id=${this.permitId}&workspace_id=${this.workspaceId}&access_token=${token}`;
    window.open(url, '_blank');
